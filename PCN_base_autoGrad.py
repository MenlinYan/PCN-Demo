...
class PCNBaseAutoGrad(nn.Module):
    ...
    def compute_energy(self, x_list, v_list):
        ...
        for i in range(self.n_layers - 1):
            energy += F.mse_loss(v_list[i+1], x_list[i+1], reduction='sum')
        return energy
    ...
    def train_step(self, x, y, n_iter=50, lr_x=0.1, lr_w=0.001):
        x_list, v_list = self.inference(x, y, n_iter, lr=0.5, visualize=True)
        E = self.compute_energy(x_list, v_list)
...
if __name__ == "__main__":
    model = PCNBaseAutoGrad([2, 2, 2, 2])
    x = torch.randn(2,2)*10
    y = torch.randn(2,2)*10
    model.fit([(x, x)], batch_size=2, n_epochs=20, n_iter=200)
